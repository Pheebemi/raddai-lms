...
export const dashboardApi = {
  getStats: async (academicYear?: string): Promise<DashboardStats> => {
    ...
    return {
      ...
      per_term_fee: data.per_term_fee,
      academic_year_id: data.academic_year_id,
      averageAttendance: data.average_attendance,
      topPerformers: data.top_performers,
      // Map recent results snake_case → camelCase
      recentResults: (data.recent_results || []).map((r: any) => ({
        id: r.id,
...
export const feesApi = {
  ...
  recordManualPayment: async (payload: {
    ...
    const response = await fetch(`${API_BASE_URL}/payments/record-manual/`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify(payload),
    });
